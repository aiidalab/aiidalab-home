...
from home import computational_resources
from home.computational_resources import (
        ...
        "mc": {
            "computer": {
                "computer-setup": {
                    ...
                    "description": "Piz Daint multicore partition",
                    ...
                    "prepend_text": "#SBATCH --partition={{ slurm_partition }}\n#SBATCH --account={{ slurm_account }}\n#SBATCH --hint={{ multithreading_hint }}",
                    "append_text": "",
                    "metadata": {
                        "template_variables": {
                            "label": {
                                "type": "text",
                                "key_display": "Computer name",
                                "default": "daint-mc",
                            },
                            "slurm_partition": {
                                "type": "list",
                            ...
                            "multithreading_hint": {
                                ...
                                "options": ["nomultithread", "multithread"],
            ...
            "codes": {
                "QuantumESPRESSO": {
                    ...
                    "description": "QE {{ code_binary_name }} binary on daint-mc",
                    "default_calc_job_plugin": "quantumespresso.{{ code_binary_name }}",
                    "filepath_executable": "/opt/qe/7.1/bin/{{ code_binary_name }}.x",
                    "prepend_text": "module load QuantumESPRESSO",
                    "append_text": "",
                    "metadata": {
                        "template_variables": {
                            "code_binary_name": {
                ...
                "cp2k-9.1": {
                    "label": "cp2k-9.1",
                    "description": "CP2K on daint-mc",
                    "default_calc_job_plugin": "cp2k",
                    "filepath_executable": "/opt/cp2k/cp2k.psmp",
                    "prepend_text": "",
                    "append_text": "",
                },
            },
        ...
        "gpu": {
            "computer": {
                "computer-setup": {
                    "label": "{{ label }}",
                    "hostname": "daint.cscs.ch",
                    "description": "Piz Daint GPU partition",
                    "work_dir": "/scratch/snx3000/{username}/aiida_run",
                    "mpirun_command": "srun -n {tot_num_mpiprocs}",
            ...
            "codes": {
                "cp2k-9.1": {
                    "label": "cp2k-9.1-gpu",
                    "description": "CP2K on daint-gpu",
                    "default_calc_job_plugin": "cp2k",
                    "filepath_executable": "/opt/cp2k/cp2k_gpu.psmp",
                    "prepend_text": "",
                    "append_text": "",
                },
            },
        ...
        "cpu": {
            "computer": {
                "computer-setup": {
                    ...
                    "description": "Merlin7 CPU partition",
            ...
            "codes": {
                "QuantumESPRESSO-7.4": {
                    ...
                    "description": "QE {{ code_binary_name }} binary on merlin7-cpu",
                    "default_calc_job_plugin": "quantumespresso.{{ code_binary_name }}",
                    "filepath_executable": "/opt/qe/7.4/bin/{{ code_binary_name }}.x",
                    "prepend_text": "",
                    "append_text": "",
                    "metadata": {
                        "template_variables": {
...
class MockResponse:
    ...
    def json(self):
        return pycopy.deepcopy(self._payload)


@pytest.fixture(autouse=True)
def mock_resource_registry(monkeypatch):
    monkeypatch.setattr(
        computational_resources.requests,
        "get",
        lambda *_args, **_kwargs: MockResponse(RESOURCE_REGISTRY_PAYLOAD),
    )
...
def pw_code(aiida_local_code_factory):
    return aiida_local_code_factory(
        label="pw", executable="bash", entry_point="core.arithmetic.add"
    )
...
def test_resource_setup_widget_for_password_configure(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "home.computational_resources.SshComputerSetup.thread_ssh_copy_id",
        lambda _: None,
    )
    w._on_quick_setup()
...
def test_computational_resources_database_widget():
    """Test the computational resources database widget."""
    ...
    assert "merlin7.psi.ch" not in widget.database
...
def test_resource_database_widget_recognize_template_entry_points():
    """Test that template-like entry points are recognized."""
    widget = ComputationalResourcesDatabaseWidget()
    assert "daint.cscs.ch" in widget.database
